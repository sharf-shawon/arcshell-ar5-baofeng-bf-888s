---
layout: default
title: Frequencies
nav_order: 6
has_children: true
description: "Frequency charts and reference guides for BaoFeng BF-888S and Arcshell AR-5 radios."
---

# 📊 Frequency Reference

Whether you're looking for the factory defaults or want to match your radio to standard FRS walkie-talkies, these charts have the information you need.

---

## Section Guide

### [1. Default Factory Channels](default-channels.md)

The 16 frequencies that come pre-programmed on most BaoFeng/Arcshell radios. **(Warning: Most are illegal to use!)**

### [2. FRS/GMRS Chart](frs-gmrs-chart.md)

Standard frequency list for consumer "walkie-talkies" used in the USA.

### [3. Amateur Radio Bands](amateur-bands.md)

Legal UHF frequencies for licensed Ham operators (70cm band).

---

## Pro Tip: Narrow vs. Wide

Most modern consumer radios (FRS/PMR446) use **Narrowband FM (NFM)**. When programming your radio in CHIRP, set the `Mode` to `NFM` for better compatibility and clearer audio when talking to other brands.
